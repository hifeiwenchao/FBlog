...
class RestCodeForm(forms.Form):
    ...
    重置密码验证码表单
    """
    username_email = forms.CharField(label='用户名或邮箱', max_length=16, widget=forms.TextInput(attrs={'class': 'form-control'}))
...
class RestPwdForm(forms.Form):
    ...
    password2 = forms.CharField(label='确认新密码', max_length=16, widget=forms.PasswordInput(attrs={'class': 'form-control'}))
