...
from fblog.util import PageInfo
...
def user_login(request):
	...
	if request.method == "POST":
		...
		if login_form.is_valid():
			...
			# 哈希值和数据库内的值进行比对
			if user and user.password == hash_code(password):
...
def profile_edit(request, uid):
	...
	userinfo = UserInfo.objects.get(username_id=uid)
	if request.method == 'POST':
		# 更新邮箱
		...
		if profile_form.is_valid():
			...
			wechat = profile_form.cleaned_data['wechat']
			...
			return redirect('blog:edituser', id=uid)
	else:
...
def about(request):
	...
	if about:
		about.contents = markdown.markdown(
			about.contents,
			extensions=[
				'markdown.extensions.extra',
				'markdown.extensions.fenced_code',
	...
	for tag_item in tags:
		temp_list.append(tag_item.tag_name)
		temp_list.append(len(tag_item.article_set.all()))
